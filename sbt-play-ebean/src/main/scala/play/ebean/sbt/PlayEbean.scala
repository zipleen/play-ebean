...
object PlayEbean extends AutoPlugin {
  ...
  override def projectSettings = {
    val querySettings = Seq(
      ...
      playEbeanQueryGenerateFinder := false,
      playEbeanQueryGenerateFinderField := false,
      playEbeanQueryGeneratePublicWhereField := false,
    ...
    try {
      ...
      try {
        ...
        if(playEbeanQueryEnhance.value) {
          val queryTransform = new org.avaje.ebean.typequery.agent.QueryBeanTransformer(playEbeanQueryArgs.value, classLoader, playEbeanQueryModelsQueryModificationPackage.value.asJava)
